...
#항상 조금만 더 자려는 마음 때문에 매일 학교를 지각하고 있다.상근이는 모든 방법을 동원해보았지만, 조금만 더 자려는 마음은 그 어떤 것도 없앨 수가 없었다.
...
#구현
inputclock = input().split(" ")
H = int(inputclock[0])
M = int(inputclock[1])
if (H >= 0 and H <= 23) and (M >= 0 and M <= 59):
    #주어진 시간보다 45분 앞서는 시간으로 변환
    # M이 45분 보다 크다면, M-45만
    # M이 45분 보다 작다면, H-1, M+15
        #H가 0이라면? H는 23
    if M < 45:
        M += 15
        H -= 1 #0입력시 H = -1
